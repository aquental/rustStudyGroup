...
The current code (./src/Course.rs) manages a university system where the Course struct has multiple responsibilities, including managing course details and grading students.
Refactor the code so that each struct has a single responsibility, leading to more maintainable and understandable code.
Refactor the code to ensure that it adheres to the Single Responsibility Principle by distributing responsibilities across multiple structs. Specifically, you should split the grading functionality into a separate struct, such as a GradeManager, and remove the grading-related methods and data from the Course struct.
 */
use std::collections::HashMap;

// TODO: Remove the grades field and methods from Course, and create a separate GradeManager struct to handle grading.
